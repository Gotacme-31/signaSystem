...
import { filterInventoryRows, inventoryMovementLabel, isLatestInventoryRequest, signedQuantity, type InventoryFilter } from "../lib/inventory";
...
export default function AdminInventory() {
  ...
  return (
    <div className="p-4 sm:p-6">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-200"><thead className="bg-slate-50 text-left text-xs font-bold uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Producto</th><th className="px-5 py-3">Control</th><th className="px-5 py-3">Stock total</th><th className="px-5 py-3">Estado</th><th className="px-5 py-3">Último movimiento</th><th className="px-5 py-3 text-right">Acciones</th></tr></thead><tbody className="divide-y divide-slate-100">
          {visibleRows.map((row) => {
            ...
            return <tr key={row.branchProductId} className="text-slate-800">
              ...
              <td className="px-5 py-4 text-sm text-slate-500">{inventory?.lastMovement ? `${inventoryMovementLabel(inventory.lastMovement.movementType)} · ${new Date(inventory.lastMovement.createdAt).toLocaleDateString("es-MX")}` : "—"}</td>
      ...
      {historyTitle && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/60 p-4"><div role="dialog" aria-modal="true" aria-labelledby="inventory-history-title" className="max-h-[90dvh] w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-start justify-between border-b p-5"><div><h2 id="inventory-history-title" className="text-xl font-black">Historial de movimientos</h2><p className="text-sm text-slate-500">{historyTitle}</p></div><button type="button" onClick={() => setHistoryTitle(null)}><X className="h-5 w-5" /></button></div><div className="max-h-[70dvh] overflow-y-auto p-5">{historyLoading ? <p className="text-center text-slate-500">Cargando...</p> : movements.length === 0 ? <p className="text-center text-slate-500">Sin movimientos.</p> : <div className="space-y-3">{movements.map((movement) => <article key={movement.id} className="grid gap-2 rounded-xl border p-4 sm:grid-cols-[1fr_auto]"><div><p className="font-bold">{inventoryMovementLabel(movement.movementType)}{movement.orderId ? ` · Pedido #${movement.orderId}` : ""}</p><p className="text-sm text-slate-500">{new Date(movement.createdAt).toLocaleString("es-MX")} · {movement.createdBy?.name ?? "Sistema"}</p>{movement.reason && <p className="text-sm">{movement.reason}</p>}</div><div className="text-right"><p className={`text-lg font-black ${movement.deltaQty < 0 ? "text-red-700" : "text-emerald-700"}`}>{signedQuantity(movement.deltaQty)}</p><p className="text-xs text-slate-500">{movement.stockBefore} → {movement.stockAfter}</p></div></article>)}</div>}</div></div></div>}
    </div>
  );
}
